...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="ruleml:Exists[(count(ruleml:*[matches(local-name(), '^[A-Z]')]) - count(ruleml:Var)) = 0 or count(ruleml:Var) = 0]">
    <xsl:copy>
      <xsl:apply-templates select="@*"/>
      <xsl:element name="ruleml:Var"/>
      <xsl:element name="ruleml:Atom">
  ...
  <xsl:template match="ruleml:Forall[(count(ruleml:*[matches(local-name(), '^[A-Z]')]) - count(ruleml:Var)) = 0 or count(ruleml:Var) = 0]">
    <xsl:copy>
      <xsl:apply-templates select="@*"/>
      <xsl:element name="ruleml:Var"/>
      <xsl:element name="ruleml:Atom">
