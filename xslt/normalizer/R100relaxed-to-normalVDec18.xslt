...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="r:Assert/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='formula']" mode="phase-1">
    ...
    <xsl:template match="r:Rulebase/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='formula']" mode="phase-1">
        <xsl:call-template name="wrap">
            <xsl:with-param name="tag">formula</xsl:with-param>
        </xsl:call-template>
    </xsl:template>   
    ...
    <xsl:template match="r:Equivalent/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='torso']" mode="phase-1">
        <xsl:call-template name="wrap">
    ...
    <xsl:template
        match="r:Retract/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='formula']"
        mode="phase-1">
        <xsl:call-template name="wrap">
            <xsl:with-param name="tag">formula</xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template
        match="r:Query/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='formula']"
        mode="phase-1">
        <xsl:call-template name="wrap">
            <xsl:with-param name="tag">formula</xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template
        match="r:And/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='formula']"
        mode="phase-1">
        <xsl:call-template name="wrap">
            <xsl:with-param name="tag">formula</xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template
        match="r:Or/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='formula']"
        mode="phase-1">
        <xsl:call-template name="wrap">
            <xsl:with-param name="tag">formula</xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template
        match="r:Neg/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='strong']"
        mode="phase-1">
        <xsl:call-template name="wrap">
    ...
    <xsl:template
        match="r:Naf/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='weak']"
        mode="phase-1">
        <xsl:call-template name="wrap">
    ...
    <xsl:template match="r:Atom/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='op' and local-name(.)!='arg' 
        and local-name(.)!='slot' and local-name(.)!='degree' and local-name(.)!='repo' and local-name(.)!='resl']" mode="phase-1">
        <xsl:choose>
    ...
    <xsl:template match="r:Implies/*[namespace-uri(.)='http://ruleml.org/spec' and position()=last()-1]" mode="phase-1">
    ...
    <xsl:template match="r:Implies/*[namespace-uri(.)='http://ruleml.org/spec' and position()=last()]" mode="phase-1">
    ...
    <xsl:template match="r:Forall/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='declare' and local-name(.) != 'formula']" mode="phase-1">
        <xsl:choose>
    ...
    <xsl:template match="r:Exists/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='declare' and local-name(.) != 'formula']" mode="phase-1">
        <xsl:choose>
            <xsl:when test="local-name(.)='Var'">
                <xsl:call-template name="wrap">
                    <xsl:with-param name="tag">declare</xsl:with-param>
    ...
    <!-- Wraps the naked RuleML children of Entails in the cases where:
        -the only children of Entails are <left>, <right> and <oid> which can appear before <if> and <then> - does not handle foreign elements in the last or second to last position
        -if both children are already wrapped then they will be copied unchanged
        -neither children are wrapped, then the second to last child is wrapped in <if> and the last child is wrapped <then>
        -the second to last child is wrapped in <then>, and the last child is not wrapped, then the last child is wrapped in <if>, in all other cases,
        the last child is wrapped in <then>
    ...
    <!-- Wraps the 1st naked RuleML child of Entails. -->
    <xsl:template match="r:Entails/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and position()=1 and local-name(.)!='if' and local-name(.)!='then']" mode="phase-1">
        <xsl:call-template name="wrap">
            <xsl:with-param name="tag">if</xsl:with-param>
        </xsl:call-template>
    </xsl:template> 
    
    <!-- Wraps the 2nd naked RuleML child of Entails. -->
    <xsl:template match="r:Entails/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and position()=2 and local-name(.)!='then' and local-name(.)!='if']" mode="phase-1">
        <xsl:if test="name(preceding-sibling::*[1]) = 'if'">     
            <xsl:call-template name="wrap">
                <xsl:with-param name="tag">then</xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        
        <xsl:if test="name(preceding-sibling::*[1]) = 'then'">     
            <xsl:call-template name="wrap">
                <xsl:with-param name="tag">if</xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="name(preceding-sibling::*[1]) != 'then' and name(preceding-sibling::*[1]) != 'if'">     
            <xsl:call-template name="wrap">
                <xsl:with-param name="tag">then</xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template match="r:Expr/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='op' and local-name(.)!='arg' 
        and local-name(.)!='slot' and local-name(.)!='degree' and local-name(.)!='repo' and local-name(.)!='resl']" mode="phase-1">
        <xsl:choose>
    ...
    <xsl:template match="r:Plex/*[namespace-uri(.)='http://ruleml.org/spec' and local-name(.)!='oid' and local-name(.)!='op' and local-name(.)!='arg' 
        and local-name(.)!='slot' and local-name(.)!='degree' and local-name(.)!='repo' and local-name(.)!='resl']" mode="phase-1">
        <xsl:call-template name="wrap">
            <xsl:with-param name="tag">arg</xsl:with-param>
        </xsl:call-template>
    ...
    <!-- Named template that wraps an element in the element given by the tag parameter. Foreign elements are copied unchanged. -->
    ...
    <xsl:template match="r:Retract" mode="phase-2">
        <xsl:copy>
            ...
            <xsl:apply-templates select="*[namespace-uri(.)!='http://ruleml.org/spec' or (local-name()!= 'oid' and local-name()!= 'formula')]" mode="phase-2"/>
        </xsl:copy> 
    </xsl:template>
    ...
    <xsl:template match="r:Entails" mode="phase-2">
        <xsl:copy>
            ...
            <xsl:apply-templates select="*[namespace-uri(.)!='http://ruleml.org/spec' or (local-name()!= 'oid' and local-name()!= 'if' and local-name()!= 'then')]" mode="phase-2"/>
        </xsl:copy>    
    </xsl:template>
    ...
    <xsl:template match="r:Exists" mode="phase-2">
        <xsl:copy>
            <xsl:apply-templates select="@*"/>
            <xsl:apply-templates select="r:oid" mode="phase-2"/>
            <xsl:apply-templates select="r:declare" mode="phase-2"/>
            <xsl:apply-templates select="r:formula" mode="phase-2"/>
            <xsl:apply-templates select="*[namespace-uri(.)!='http://ruleml.org/spec' or (local-name()!= 'oid' and local-name()!= 'declare' and local-name()!= 'formula')]" mode="phase-2"/>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="r:Equivalent" mode="phase-2">
        <xsl:copy>
            ...
            <xsl:apply-templates select="*[namespace-uri(.)!='http://ruleml.org/spec' or (local-name()!= 'oid' and local-name()!= 'torso')]" mode="phase-2"/>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="r:Atom" mode="phase-2">
        <xsl:copy>
            ...
            <xsl:apply-templates select="@*|*[namespace-uri(.)!='http://ruleml.org/spec' or (local-name()!= 'oid' and local-name()!= 'op' and local-name()!= 'arg' and local-name()!='repo' and local-name()!='slot' and local-name()!='resl')]" mode="phase-2"/>
        </xsl:copy> 
    </xsl:template>
    ...
    <xsl:template match="r:Equal" mode="phase-2">
        <xsl:copy>
            ...
            <xsl:apply-templates select="*[namespace-uri(.)!='http://ruleml.org/spec' or (local-name()!= 'oid' and local-name()!= 'left' and local-name()!= 'right')]" mode="phase-2"/>
        </xsl:copy> 
    </xsl:template>
    ...
    <xsl:template match="r:Neg" mode="phase-2">
        <xsl:copy>
            ...
            <xsl:apply-templates select="*[namespace-uri(.)!='http://ruleml.org/spec' or (local-name()!= 'oid' and local-name()!= 'strong')]" mode="phase-2"/>
        </xsl:copy>     
    </xsl:template>
    ...
    <xsl:template match="r:Naf" mode="phase-2">
        <xsl:copy>
            ...
            <xsl:apply-templates select="*[namespace-uri(.)!='http://ruleml.org/spec' or (local-name()!= 'oid' and local-name()!= 'weak')]" mode="phase-2"/>
        </xsl:copy>     
    </xsl:template>
    ...
    <xsl:template match="r:Expr" mode="phase-2">
        <xsl:copy>
            ...
            <xsl:apply-templates select="r:slot" mode="phase-2"/>
            <xsl:apply-templates select="r:resl" mode="phase-2"/>
            <xsl:apply-templates select="*[namespace-uri(.)!='http://ruleml.org/spec' or (local-name()!= 'oid' and local-name()!= 'op' and local-name()!= 'arg' 
                and local-name()!='repo' and local-name()!='slot' and local-name()!='resl')]" mode="phase-2"/>
        </xsl:copy>     
    </xsl:template>
    ...
    <xsl:template match="r:Plex" mode="phase-2">
        <xsl:copy>
            ...
            <xsl:apply-templates select="r:resl" mode="phase-2"/>
            <xsl:apply-templates select="*[namespace-uri(.)!='http://ruleml.org/spec' or (local-name()!= 'oid' and local-name()!= 'op' and local-name()!= 'arg' 
                and local-name()!='repo' and local-name()!='slot' and local-name()!='resl')]" />
        </xsl:copy>     
    </xsl:template>
    <!-- copy comments-->
