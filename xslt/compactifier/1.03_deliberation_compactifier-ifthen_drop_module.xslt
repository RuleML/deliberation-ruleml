...
<xsl:import href="1.03_deliberation_compactifier-ifthen_module.xslt"/>  
  ...
  <!-- Drop the xsi:schemaLocation attribute on a skippable element to the child node,
    if it doesn't have one already, when skipping -->
  <xsl:template match="ruleml:*[ruleml:isSkippableIfThenEdge(..)][not(.[@xsi:schemaLocation])]" mode="phase-compactify">
