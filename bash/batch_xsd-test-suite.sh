...
TEST_SUITE_HOME=${RNC_HOME}/../test/xsd-test-suites

#for file in ${TEST_HOME}/*.xsd
#do
#  ${BASH_HOME}/aux_valxsd.sh $file
#  if [ "$?" -ne "0" ]; then
...
do
  ...
  while read -r; do
     ...
     then     
       ...
       ${BASH_HOME}/aux_valxsd.sh ${TEST_HOME}/${schemaname}
       exitvalue=$?
       if [ "${exitvalue}" -ne "0" ]; then
          echo "Schema Validation Failed for " "${file}"
          exit 1
       fi   
       ${BASH_HOME}/aux_valxsd.sh ${TEST_HOME}/${schemaname} ${file}
       exitvalue=$?
