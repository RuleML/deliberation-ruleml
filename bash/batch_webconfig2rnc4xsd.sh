...
do
    ...
    tokens=( $line )
    "${BASH_HOME}aux_web2rnc.sh" "${tokens[1]}" "${RNC4XSD_NORMAL_HOME}${tokens[0]}_normal.rnc"
...
do
    echo "Text read from file - $line"
    tokens=( $line )
    "${BASH_HOME}aux_web2rnc.sh" "${tokens[1]}" "${RNC4XSD_NORMAL_MIN_HOME}${tokens[0]}_normal.rnc"
...
cp "${RNC_HOME}hornlogrepoexpr_normal4xsd.rnc" "${RNC4XSD_HOME}hornlogrepoexpr_normal.rnc"
cp "${RNC_HOME}hornlogrepoexpr4xsd.rnc" "${RNC4XSD_HOME}hornlogrepoexpr.rnc"
cp "${RNC_HOME}naffologeqPSOA_compact4xsd.rnc" "${RNC4XSD_HOME}naffologeqPSOA_compact.rnc"
